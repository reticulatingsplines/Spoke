...
export async function exportCampaign(job) {
  ...
  const appendToKey = (array, appendString) => {
    if ((array.length = 0)) {
      return;
    }
    ...
    Object.keys(array).forEach(key => {
      array[`${appendString}${key[0].toUpperCase() + key.substring(1)}`] =
        array[key];
      delete array[key];
    });
  };
  ...
  const contacts = await r
    ...
    .column([
      ...
      { contactExternalID: "campaign_contact.external_id" },
      { contactFirstName: "campaign_contact.first_name" },
      { contactLastName: "campaign_contact.last_name" },
      { contactCellNumber: "campaign_contact.cell" },
      { contactCityFromZip: "zip_code.city" },
      { contactStateFromZip: "zip_code.state" },
      { contactZip: "campaign_contact.zip" },
  ...
  console.log(`contacts is type ${typeof contacts}`);
  console.log(`JSON stringified its ${JSON.stringify(contacts)}`);

  contacts.forEach((row, index) => {
    // Split Custom fields into columns and append to keys
    console.log(`texterFirst is ${row.texterFirst}`);
    const customContactFields = JSON.parse(row.custom_fields);
    delete row.custom_fields;
    if (customContactFields) {
      appendToKey(customContactFields, "contact");
      delete customContactFields.contactLength;
    }


    // Move texter data to const to use later to append on right

    const texterFields = {
      texterFirst: row.texterFirst,
      texterLast: row.texterLast,
      texterEmail: row.texterEmail,
      texterCell: row.texterCell,
      texterAlias: row.texterAlias
    };

    delete row.texterFirst;
    delete row.texterLast;
    delete row.texterEmail;
    delete row.texterCell;
    delete row.texterAlias;
  ...
  const messages = await r
    ...
    .column([
      ...
      { texterAlias: "user.alias" },
      "user.extra",
      { texterNumberUsed: "user_number" },
      { contactFirstName: "campaign_contact.first_name" },
      { contactLastName: "campaign_contact.last_name" },
      { contactNumber: "contact_number" },
      { contactZip: "campaign_contact.zip" },
      { contactCustom: "campaign_contact.custom_fields" },
      "is_from_contact",
      "send_status",
      { attempted_at: "message.created_at" },
      "text",
      { messageErrors: "message.error_code" }
    ])
    .select()
    .where("campaign_contact.campaign_id", campaign.id);

  messages.forEach((row, index) => {
    let customContactFields_mesg = JSON.parse("{}");
    if (row.contactCustom) {
      const customContactFields_mesg = JSON.parse(row.contactCustom);
      delete row.contactCustom;
      appendToKey(customContactFields_mesg, "contact");
      delete customContactFields_mesg.contactLength;
    }

    //split rows we'd like left of custom user fields
    row.texterNumberUsed = row.texterNumberUsed || "NA";
    const texterFields_mesg = {
      ...
      texterEmail: row.texterEmail,
      texterAlias: row.texterAlias,
      texterNumberUsed: row.texterNumberUsed
    };

    delete row.campaign_contact_id;
    delete row.campaign_id;
    delete row.texterFirst;
    delete row.texterLast;
    delete row.texterEmail;
    delete row.texterAlias;
    delete row.texterNumberUsed;

    //split rows we'd like right of custom contact fields
    const messageData = {
      is_from_contact: row.is_from_contact,
      send_status: row.send_status,
      attempted_at: row.attempted_at.toLocaleString("en-US", {
        timeZone: "America/New_York",
        timeZoneName: "short"
      }),
      text: row.text,
      messageErrorCodes: row.messageErrors
    };

    delete row.is_from_contact;
    delete row.send_status;
    delete row.attempted_at;
    delete row.text;
    delete row.messageErrors;

    messages[index] = {
      ...texterFields_mesg,
      ...customUserFields_mesg,
      ...row,
      ...customContactFields_mesg,
      ...messageData
    };
    delete messages[index]["extra"];
  });
  ...
  if (
    ...
    const contactsFile = `./${moment()
      .utc()
      .format("YYYY-MM-DD-HHmm")}UTC_orgID_${campaign.organizationId}_export_${
      campaign.title
    }.csv`.replace(" ", "_");
    const messagesFile = `./${moment()
      .utc()
      .format("YYYY-MM-DD-HHmm")}UTC_orgID_${campaign.organizationId}_export_${
      campaign.title
    }_message.csv`.replace(" ", "_");
